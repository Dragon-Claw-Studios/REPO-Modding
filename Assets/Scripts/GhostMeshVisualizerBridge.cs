...
public static class GhostMeshVisualizerBridge
{
#if UNITY_EDITOR
    ...
    private static void EnsureInit()
    {
        ...
        if (foundType != null)
            _propInfo = foundType.GetProperty("GhostMesh", BindingFlags.Public | BindingFlags.Static);

        if (_propInfo != null)
            _getter = () => (bool)_propInfo.GetValue(null);
        else
            _getter = () => EditorPrefs.GetBool("GhostMeshVisualizerToggle", false);
    }
