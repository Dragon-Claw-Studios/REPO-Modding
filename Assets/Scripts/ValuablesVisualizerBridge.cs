...
public static class ValuablesVisualizerBridge
{
#if UNITY_EDITOR
    ...
    private static void EnsureInit()
    {
        ...
        if (foundType != null)
            _propInfo = foundType.GetProperty("Valuables", BindingFlags.Public | BindingFlags.Static);

        if (_propInfo != null)
            _getter = () => (bool)_propInfo.GetValue(null);
        else
            _getter = () => EditorPrefs.GetBool("ValuablesVisualizerToggle", false);
    }
