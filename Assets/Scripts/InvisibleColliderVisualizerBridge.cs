...
public static class InvisibleColliderVisualizerBridge
{
#if UNITY_EDITOR
    ...
    private static void EnsureInit()
    {
        ...
        if (foundType != null)
            _propInfo = foundType.GetProperty("InvisibleCollider", BindingFlags.Public | BindingFlags.Static);

        if (_propInfo != null)
            _getter = () => (bool)_propInfo.GetValue(null);
        else
            _getter = () => EditorPrefs.GetBool("InvisibleColliderVisualizerToggle", false);
    }
